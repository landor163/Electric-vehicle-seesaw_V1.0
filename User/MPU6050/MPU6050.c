...
void mpu_scan()
{	
	dmp_get_pedometer_step_count(&count);//调用DMP器件库的函数来获取当前的步数，需要注意的是：必须连续走7步或者7步以上才会显示步数
	printf("S%dT",count);//串口1打印步数
	//OLED_ShowStr(2,2,(unsigned char *)"counting！",2);
	//OLED_ShowStr(4,4,(unsigned char *)count,2);
}
